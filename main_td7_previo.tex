...
\begin{itemize}[leftmargin=*]
    \item \textbf{OCURRE\_EN} entre \texttt{SINIESTRO} y \texttt{TRAMO\_VIAL}: un tramo vial puede estar asociado a muchos siniestros; cada siniestro se registra en un único tramo vial principal.
    \item \textbf{GENERA} entre \texttt{SINIESTRO} e \texttt{INFORME\_TECNICO}: un siniestro puede generar cero, uno o varios informes; cada informe corresponde a un único siniestro.
...
\begin{itemize}[leftmargin=*]
    \item un script SQL para crear las tablas y restricciones;
    \item un script SQL para cargar datos de ejemplo;
    \item un script SQL con consultas de interés sobre la base.
\end{itemize}
